...
- **排版与状态**：名称 13pt Medium + 元信息 11pt 三级灰（大小 · 相对时间），
  层级只靠字重与灰度表达；分组计数是紧随组名的等宽注脚（搜索时切换为青瓷
  `匹配/总数` 计数），变化时数字滚动；排序按钮的图标即当前排序模式
  （时钟环 / 沙漏 / abc / 网格 / 把手），状态一眼可读；置顶区与普通区之间
  有一道 hairline 分界。
- **动效**：收起/展开是真实的弹簧物理（`WindowSpringAnimator` 数值积分驱动窗口 frame，
  ...
  列表整体轻退后让位给新文件；空态虚线环缓慢旋转、圆盘呼吸浮动；
  悬停操作钮错峰滑入、按钮按压缩放；撤销 / 轻提示弹出时列表底部同步让位，
  最后一行不被遮挡；键盘 ↑↓ 移动选中时行自动滚动跟随。
