...
- 👆 **快捷操作**：双击打开文件（可在设置改为单击打开）；悬停行出现「在访达中显示 / 另存为 / 移除」；
  右键菜单含「拷贝文件」「拷贝路径」「移动到文件夹…」等全部操作；导入文件走菜单栏或主菜单（⌘O）。
- ⌫ **键盘移除**：Delete 键移除选中条目。
...
- 🍃 **菜单栏图标**：托盘图标菜单提供 显示/隐藏抽屉、导入、清空、设置、关于、退出。
...
| 放入文本/链接 | 从任意应用拖一段文字 / 链接进抽屉（自动存成 .txt / .webloc）；或复制后 **⌘V** |
