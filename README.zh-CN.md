...
> **English**: the [English README](README.md) carries the full documentation.
...
- 🖼 **图片与文件承诺也能拖入**：从网页里直接拖一张图，落到抽屉的是**图片文件本身**
  （图像数据优先于随行的 URL，不再是 `.webloc` 链接）；从照片 App 拖照片 / 视频、从邮件拖附件，
  以**文件承诺**形式到达、按原始文件名接收进收件箱；音频 / PDF 等媒体类型同样接收。
  浏览器 / 预览里「拷贝图像」后 **⌘V**，直接物化成 `.tiff` / `.png` 文件。
- ✋ **拖出取回**：按住条目往抽屉外拖即可——落到 Finder 是拷贝真实文件，放到邮件、聊天窗口等也认。
- 📦 **拖出即移走**：拖出时**按住 ⌘ 落到访达 = 移动**（访达把源文件移走，条目随之自动清掉）；
  **拖到程序坞废纸篓 = 删除**；设置里开启「拖出（拷贝）后移除条目」后，普通拷贝拖出也会
  把条目带离抽屉（可还原）。右键 **「移到废纸篓」** 把选中条目的源文件送进废纸篓；
  选中行拖瓷片到其他行上仍是排序——瓷片成为通用把手（拖出 / ⌘移动 / 排序三合一）。
- 🖱 **多选批量**：**⌘点击**逐个勾选、**⇧点击**选区间、**⌘A** 全选；
  Delete / ⌘C / ⏎ / 右键菜单全部按访达语义作用于整批，批量移除同样可一键「还原」；
  **选中时拖动瓷片可把整批文件一次拖出**（×N 角标预览，落到访达 / 邮件即多文件拷贝）。
...
- 🍃 **菜单栏快速访问**：托盘菜单含「最近条目」（最近 6 个，点击直接打开）、
  ...
  「打开收件箱文件夹」（拖入文本/链接/图片物化文件的所在目录，可直接在访达/Spotlight 检索）。
- 📋 **剪贴板互通**：**⌘C** 把选中条目按「访达拷贝」语义写入剪贴板（任何应用可粘贴，多选拷多个）；
  **⌘V** 把剪贴板里的文件放入抽屉，剪贴板是文本 / 链接 / 图像时直接物化成条目；右键菜单含「拷贝文件」。
- 🕘 **剪贴板历史**：任意应用里复制过的 文本 / 链接 / 图像 / 文件 自动留档在抽屉内的历史面板
  （**⌘⇧V**、头部按钮或菜单栏进入），点击条目即**收进抽屉**，悬停与右键可 拷回剪贴板 / 置顶 / 删除；
  支持搜索、相对时间与来源应用标注。隐私优先：被排除的应用永不记录（默认排除钥匙串访问与
  1Password 等密码管理器，任意条目右键即可排除其来源应用）、自身写回不回声、图像超 8MB 不入档，
  容量上限 20/50/100/200 可选（置顶豁免淘汰）。
...
| ⌘1 … ⌘9 | 切换到第 N 个分组 |
| ⏎ | 打开选中文件（多选时全部打开） |
| ⌘↓ | 打开选中文件（与 ⏎ 相同，对齐系统列表惯例） |
| ⌘Z | 撤销最近一次移除（编辑菜单同入口；无快照时禁用） |
| ⌘↵ | 预览打开时：用默认应用打开当前条目并关闭预览 |
| ⌘C | 拷贝选中条目的文件（与访达拷贝同构，多选拷多个） |
| ⌘V | 把剪贴板的文件 / 文本 / 链接 / 图像放入抽屉 |
| ⌘⇧V | 打开 / 关闭剪贴板历史 |
...
| 放入文本/链接/图片 | 从任意应用拖一段文字 / 链接进抽屉（自动存成 .txt / .webloc）；从网页拖图、从照片 App 拖照片（存成图片文件本身）；或复制后 **⌘V**（富文本自动转纯文本，拷贝的图像直接成文件） |
...
| 4 | App Store Connect 生成 App 专用密码 | [notary](https://notary.apple.com)（存入钥匙串：`xcrun notarytool store-credentials`） |
...
├── ClipboardHistory.swift   # 剪贴板历史（捕获 / 存储 / 监控 / 应用排除 / 收进抽屉）
├── ClipboardHistoryView.swift # 剪贴板历史面板 UI（搜索 / 置顶 / 拷回 / 收进抽屉）
...
├── DragOutSupport.swift     # 选中拖出：NSDraggingSession 拖拽源（×N 角标、拷贝/移动/删除语义）
...
Sources/FileDrawer/Resources/{zh-Hans,en}.lproj/
                             # 本地化表（英文值；中文 key 即原文）
Tests/FileDrawerTests/       # 252 个测试：拖拽往返 / 多选 / 分组与置顶 / 每组排序 /
...
- **拖入**：`.onDrop(of: [.fileURL, .url, .utf8PlainText, .plainText] + 媒体类型(image/movie/audio/pdf))`，
  逐个解析 provider（优先 `loadObject(ofClass: URL.self)`，兜底原始数据），批量入列并去重。
  文件 URL 原地引用；**图像数据**（浏览器拖图：图像数据优先于随行 URL，落图本身而非 `.webloc`）、
  **文件承诺**（`loadFileRepresentation` 按原始文件名接收照片 / 邮件附件式承诺文件，
  系统不 coerce 的 provider 走原始数据兜底）与文本 / 链接一样先经 `InboxStore` 物化成
  收件箱里的真实文件再入列——排序、缩略图、拖出、QuickLook 对它们与普通文件完全一致；
  富文本粘贴走 RTF→纯文本回退。
